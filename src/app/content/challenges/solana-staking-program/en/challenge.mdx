...
2. **Unstake** — User **burns** \$STAKE; the program **transfers** the corresponding \$BEEF from the vault back to the user.
3. **Rewards System** - Use SushiSwap MasterChef Rewards Design Model, four specific requirements about the rewards system:
 - **New Reward Token**: We will need to create a new SPL token for the rewards (e.g., $MILK or whatever fun name you prefer!).
 - **No Big Loops**: When calculating user rewards, please avoid using large loops to iterate through all users, as that will easily hit the compute unit (gas) limits on-chain.
 - **Dynamic Reward Emission**: The design should account for dynamic changes in the total pool reward distribution over time. For example, in real-world scenarios, the emission rate is usually higher when the pool is first created, and gradually decreases as the pool gets older. How would you implement this dynamic rate?
 - **Separation of Principal and Rewards**: The staking principal and the generated rewards need to be decoupled. A user should be allowed to interact with just their principal (e.g., unstake a portion of their staked tokens) without being forced to claim rewards, and they can simply claim their accumulated rewards later.
...
#### 4. Matching Frontend UI

- **Frontend:** Provide a frontend UI (e.g. with wallet-adapter) that allows users to connect a wallet, stake input tokens, and unstake (burn stake tokens and receive input tokens back) on devnet. The UI should demonstrate the same stake/unstake flow as the program.

#### 5. Program Security

- **Account validation:** All instruction accounts are validated (correct owners, mints, token accounts). Only the program’s PDA can act as mint authority and vault authority.
- **Signer checks:** User is required as signer for stake (transfer) and unstake (burn); no one can move another user’s tokens without authorization.
- **Vault & CPI safety:** Outgoing transfers from the vault use the vault PDA as signer; no other account can drain the vault. Amounts are checked to prevent over-mint, over-transfer, or over-burn (e.g. no overflow/underflow, balance checks where relevant).
- **Documentation:** Briefly document how the program enforces the above (e.g. in README or comments).

#### 6. Technical Design Document

- **Design doc:** Produce a technical design document that describes architecture, account model (mints, vault, PDAs), instruction flows (stake / unstake), CPI usage, and security considerations. It should be sufficient for a reviewer to understand the solution before reading the code.

#### 7. Code Review

- **Final code review:** Organize and run a code review meeting (e.g. with a mentor or peer) to walk through the program and frontend. Submit or document the outcome (e.g. review notes, action items, or sign-off) as part of the challenge deliverable.
